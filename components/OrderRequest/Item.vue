<template>
  <div class="flex flex-col sm:flex-row justify-between p-5 sm:p-6 lg:p-[30px] gap-y-5 md:gap-y-0">
    <div class="flex gap-x-4 md:gap-x-6">
      <img :src="detail.productImg || '/images/imgplace.png'" alt="img" width="117" height="117"
      ...
      <div class="w-full sm:w-auto sm:max-w-[350px]">
        ...
        <p class="text-[10px] md:text-xs mb-[10px]md: mb-[15px]">
          <span class="font-normal">Manufactured by:</span><span class="font-bold"> {{ detail?.producer }}</span>
        </p>
        <div class="flex flex-col sm:flex-row gap-y-4 lg:gap-y-0 sm:gap-x-6">
          <div>
            ...
            <span class="text-sm rounded border border-[#E7E7E7] bg-white flex items-center h-8 px-[15px]">{{
              detail.selectedPackage }}</span>
          </div>
          <div>
            <p class="font-medium text-[10px] md:text-xs mb-1">Quantity</p>
            <div class="h-8 sm:max-w-[115px]">
              <CartButton textClass="!text-[11px] md:!text-sm" iconClass="!text-[10px] md:!text-xs"
                btnClass="!px-[10px]" />
            </div>
          </div>
        </div>
    ...
    <div class="flex flex-col justify-center md:justify-between">
      ...
      <AppButton @click="orderRequestStore?.removeFromRequest(detail.id)" text="Remove item" icon="bx:trash"
