<template>
  <form class="px-4 lg:px-[30px]" @submit.prevent="handleSubmit">
    ...
    <div class="mb-6">
      <label for="" class="mb-2 font-medium text-sm text-[#344054] text-left flex items-center gap-x-1">
        <span>Pickup location </span>
      </label>

      <VueSelect v-model="v$.pickUpLocationId.$model" :options="locations" :reduce="(location) => location.value"
        placeholder="Select location"
        :classInput="`min-w-[180px] !bg-white  !rounded-lg !text-[#475467] !h-11 cursor-pointer border-[#D0D5DD]`" />
      <div class="flex justify-start mt-1">
        <button @click="isLocationOpen = true" class="text-xs text-primary font-medium" type="button">
          + Add a new location
        </button>
      </div>
      <div class="text-red-500 mt-1" v-for="error of v$.pickUpLocationId.$errors" :key="error.$uid">
    ...
    <div class="flex gap-x-[56px] justify-start flex-col lg:flex-row gap-y-7 lg:gap-y-10">
      ...
      <div class="max-w-[654px] w-full">
        ...
        <div class="text-red-500 mt-1" v-for="error of v$.packagesAvailable.$errors" :key="error.$uid">
          <div class="error-msg text-error text-xs font-semibold">
            {{ error.$message }}
          </div>
        </div>
        <div class="border border-[#DCDEE6] rounded-[10px] mt-6">
          <table class="w-full">
            <thead>
              <tr>
                <th v-for="(item, i) in headers" :key="item + i"
            ...
            <tbody v-if="form.packagesAvailable?.length">
              <tr v-for="item in form.packagesAvailable" :key="item.id"
                ...
                <td class="capitalize text-matta-black text-sm font-normal py-4 px-4 whitespace-nowrap">
                  {{ item?.package?.title }}
                </td>
                <td class="capitalize text-matta-black text-sm font-normal py-4 px-4 whitespace-nowrap">
                  {{ item?.size }}{{ item?.unit }}
                ...
                <td class="capitalize text-matta-black text-sm font-normal py-4 px-6 ] whitespace-nowrap">
          ...
          <EmptyData v-if="!form.packagesAvailable?.length" title="No package added yet"
            titleClass="text-xs !font-normal text-[#475467]" className="!h-auto py-6" classIcon="!text-3xl w-[120px]" />
        </div>
      </div>
    </div>
    ...
    <section class="px-6 lg:px-[30px] py-[30px]">
      <div class="flex gap-x-[56px] justify-start text-left flex-col lg:flex-row gap-y-7 lg:gap-y-10">
        ...
        <div class="max-w-[654px] w-full">
          ...
          <div class="bg-white py-6 lg:py-8 rounded-lg" v-if="form.documents.length">
            <div class="">
              <div class="flex items-center gap-x-4 mb-3" v-for="(n, id) in form.documents" :key="id">
                <div class="border rounded-xl p-4 flex flex-1 justify-between">
                  <div class="flex gap-x-3 items-center">
                    <img src="~/assets/images/filetype.png" alt="prduct" class="w-8 h-auto" />
                    <div>
                      <p class="text-sm text-matta-black capitalize truncate max-w-[250px]">
                        {{ n.fileName }}
                      </p>
                      ...
                      <Listbox v-model="n.category">
                        <div class="relative mt-1">
                          <ListboxButton
                            ...
                            <span class="mr-2 capitalize max-w-[130px] truncate" v-if="n.category">{{ n.category
                              }}</span>
                            <span class="text-matta-black mr-2 whitespace-nowrap" v-else>Select category</span>
                          ...
                          <transition leave-active-class="transition duration-100 ease-in"
                            ...
                            <ListboxOptions
                              ...
                              <ListboxOption v-slot="{ selected }" v-for="(p, i) in form.documentproperties" :key="i"
                                :value="p.value" as="template">
                                <li :class="[
                                  selected
                                  ...
                                  <div class="flex gap-x-4 items-start">
                                    ...
                                    <i class="uil uil-check text-blue-800" v-if="selected"></i>
                                  </div>
                                </li>
                              </ListboxOption>
                              <p v-if="!form.documentproperties.length" class="text-[#B6B7B9] py-2 pl-6 pr-4 text-sm">
                                Nothing found
                              </p>
                            </ListboxOptions>
                          </transition>
    ...
    <div class="bg-white rounded-lg py-6 flex justify-between gap-x-10 items-center">
      ...
      <button type="button" @click="toggleNext(1)"
        class="appearance-none leading-none px-10 py-[10px] rounded-lg text-primary border-primary- border hover:bg-gray-300 text-[13px]">
        Back
      </button>
  ...
  <div>
    <Modal :isOpen="isAddingPackage" @toggleModal="isAddingPackage = false" :canClose="false">
      <template #content>
        <form v-if="typeForm === 'producer'" class="bg-white px-4 pt-5 pb-8 sm:p-6 sm:pb-4 w-[500px] rounded-lg"
          @submit.prevent="handleProducer">
          <div class="flex justify-between mb-8 items-center">
            <h4 class="font-medium text-matta-black text-xl">Add Producer</h4>
            <i class="uil uil-times cursor-pointer text-lg hover:ring-1 w-6 h-6 ring-gray-200 flex items-center justify-center hover:ring-offset-2 rounded-full"
              @click="isAddingPackage = false"></i>
          </div>
          <div class="mb-5">
            <label for="" class="mb-2 font-normal text-xs block">Name <span
                class="text-red-500 pl-[.5px]">*</span></label>
            <input v-model="producerForm.title"
          ...
          <div class="flex gap-x-6 mb-5">
            <div class="w-full">
              <label for="" class="mb-2 font-medium text-sm text-[#344054] block text-left">Country</label>
              <CountriesSelect v-model="producerForm.country" />
            </div>
            <div class="w-full">
              <label for="" class="mb-2 font-medium text-sm text-[#344054] block text-left">State</label>
              <StatesSelect v-model="producerForm.state" :states="states" />
            </div>
          </div>
          <div>
            <label for="" class="mb-2 font-medium text-sm text-[#344054] block text-left">Producer Logo</label>
            <label for="upload" class="cursor-pointer">
              <input @change="handleEvent($event)" type="file" accept="image/*" id="upload" class="hidden" />
              <div>
                <span v-if="!producerForm.logo"
                  class="h-16 w-16 rounded-full flex items-center text-xs bg-[#F1F3F5] mr-4 justify-center">Logo</span>
                <img v-else :src="producerForm.logo"
                  class="h-16 w-16 rounded-full flex items-center bg-[#F1F3F5] mr-4 justify-center" />
              </div>
              <i class="fa fa-spinner fa-spin ml-6" v-if="isLoadingLogo" aria-hidden="true"></i>
            </label>
          </div>

          <hr class="my-6" />
          <div class="flex justify-end gap-x-2 items-center mt-8">
            <button type="button" @click="isAddingPackage = false"
        ...
        <PackageForm v-if="typeForm === 'package'" @close="() => {
